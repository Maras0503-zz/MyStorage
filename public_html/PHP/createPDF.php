<?php
    ...
    $content .= '<div class="productTableDiv">';
            ...
            if($returnArray[0]['document_type_short'] != 'PZ'){
                ...
                while ($row = $fetch->fetch_array()) {
                    $content .= '<tr>';
                        ...
                        $price = round(($price + ($price*$row['document_records_vat']/100))*100)/100;
                        $content .= '<td class="tableBodyCell col9of11content">'.$price.'</td>';
                        $price = round(($price * $numb)*100)/100;
                        $content .= '<td class="tableBodyCell col10of11content">'.$price.'</td>';
                        $price = round(($price - ($price*$disc))*100)/100;
            ...
            if($returnArray[0]['document_type_short'] != 'PZ'){
                ...
                while ($row = $fetch->fetch_array()) {
                    $content .= '<tr>';
                        ...
                        $numb = $row['document_records_product_number']/100;
                        $content .= '<td class="tableBodyCell col4of11content">'.$numb.'</td>';
                        $content .= '<td class="tableBodyCell col5of11content">'.$row['product_unit_en'].'</td>';  
                        $content .= '<td class="tableBodyCell col6of11content">'.$row['document_records_vat'].'</td>';
                        $disc = $row['document_records_discount']/100;
                        $content .= '<td class="tableBodyCell col7of11content">'.$disc.'</td>';
                        $price = $row['document_records_price']/100;
        ...
        if($returnArray[0]['document_type_short'] != 'PZ'){
            if($lang=='pl'){
                $content .= '<div class="valueSum">';
                    $content .= 'WARTOŚĆ CAŁKOWITA: '.$sum.'<br>';
                    $content .= amountInWordsPL($sum);
                $content .= '</div>';
            }
            if($lang=='en'){
                $content .= '<div class="valueSum">';
                    $content .= 'TOTAL AMOUNT: '.$sum.'<br>';
                    $content .= amountInWordsEN($sum);
                $content .= '</div>';
            }
        }
        if($returnArray[0]['document_type_short'] != 'PZ'){
            $content .= '<div class="vatTableDiv">';
                $content .= '<table class="vatTable">';
                if($lang == 'pl'){
                    $content .= '<tr>';
                        $content .= '<td class="tableBodyCell col1of3">STAWKA VAT</td>';
                        $content .= '<td class="tableBodyCell col2of3">KWOTA PODATKU</td>';
                        $content .= '<td class="tableBodyCell col3of3">WARTOŚĆ BRUTTO</td>';
                    $content .= '</tr>';
                    include('connect.php');
                    $fetch = $mysqli->query('call getVatsFromDocument('.$docId.')'); 
                    while ($row = $fetch->fetch_array()) {
                        $content .= '<tr>';
                            $content .= '<td class="tableBodyCell col1of3content">'.$row['document_records_vat'].'</td>';
                            $price = ($row['document_records_product_number']/100) * ($row['value_sum']/100) * ($row['document_records_vat']/100);
                            $content .= '<td class="tableBodyCell col2of3content">'.(round($price*100)/100).'</td>';
                            $price = (($row['document_records_product_number']/100) * ($row['value_sum']/100)) + ($row['document_records_product_number']/100) * ($row['value_sum']/100) * ($row['document_records_vat']/100);
                            $content .= '<td class="tableBodyCell col3of3content">'.(round($price*100)/100).'</td>';
                        $content .= '</tr>';
                    }
                    $mysqli->close();
                }
                if($lang == 'en'){
                    $content .= '<tr>';
                        $content .= '<td class="tableBodyCell col1of3">VAT RATE</td>';
                        $content .= '<td class="tableBodyCell col2of3">VAT AMOUNT</td>';
                        $content .= '<td class="tableBodyCell col3of3">GROSS VALUE</td>';
                    $content .= '</tr>';
                    include('connect.php');
                    $fetch = $mysqli->query('call getVatsFromDocument('.$docId.')'); 
                    while ($row = $fetch->fetch_array()) {
                ...
                $content .= '</table>';
            $content .= '</div>';
            $content .= '<div class"signPlaces">';
            if($lang == 'pl'){
                $content .= '<div class="authorizedSignature">';
                    $content .= '..............................................<br>';
                    $content .= 'Podpis osoby wystawiającej';
                $content .= '</div>';
                $content .= '<div class="recipientSignature">';
                    $content .= '..............................................<br>';
                    $content .= '      Podpis odbiorcy     ';
                $content .= '</div>';
            }
            if($lang == 'en'){
                $content .= '<div class="authorizedSignature">';
